...
// Secrets come from the environment — never hardcode keys in committed code.
// Writes now require the service_role key because Row-Level Security is locked
// down (the anon key can no longer insert). Run locally only; never commit the key:
//   SUPABASE_URL=... SUPABASE_SERVICE_KEY=... node scripts/import_inventory.mjs vans.csv
const SUPABASE_URL = process.env.SUPABASE_URL || "https://nubttdudughlriajikrr.supabase.co";
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_ANON_KEY;
if (!SUPABASE_KEY) {
  console.error("Missing key. Set SUPABASE_SERVICE_KEY (preferred) in your environment before running.");
  process.exit(1);
}
